...
| 模型 | 年份 | 结构示意 | 核心思想 |
|------|------|----------|----------|
| **FNN** | 2016 | `FM embedding → DNN` | 用预训练好的 FM 向量初始化 DNN 输入层 |
| **PNN** | 2016 | `Embedding → Product Layer → DNN` | 在 embedding 之上加显式内积/外积乘积层 |
| **NeuFM** | 2017 | `FM(线性) + DNN(非线性) → 融合` | FM 捕捉低阶 + DNN 捕捉高阶，输出层融合 |
| **AFM** | 2017 | `FM + Attention 权重` | 不同特征交叉的重要性不同，用 attention 加权 |
| **Wide&Deep** | 2016 | `Wide(手工交叉) ∥ Deep(DNN) → 联合` | Google：Wide 负责记忆，Deep 负责泛化 |
| **DeepFM** | 2017 | `FM ∥ DNN → 联合` | 用 FM 替代 Wide 中的手工特征工程，共享 embedding |
| **DCN** | 2017 | `Cross Network ∥ DNN → 联合` | 用 Cross Network 自动做有界阶数的显式交叉 |
| **xDeepFM** | 2018 | `CIN(向量级交叉) ∥ DNN → 联合` | 交叉粒度从标量提升到向量级，表达力更强 |
| **AutoInt** | 2019 | `Embedding → Multi-head Self-Attention` | 直接用自注意力建模任意阶特征交互 |
